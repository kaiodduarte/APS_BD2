...
public class Transacao {
    ...
    public Transacao(ListaDados dados, int numeroAcessos, int ultimoIndice) {
        ...
        filaOperacoes.add(new Operacao(Acesso.BEGIN, ultimoIndice));
        randomizaOperacoes(dados.getDados(), numeroAcessos);
        filaOperacoes.add(new Operacao(Acesso.END, ultimoIndice));
    }

    private void randomizaOperacoes(LinkedList<Dado> dados, int numeroAcessos) {
        int tamanhoConjunto = dados.size();
        int[] vetorPosicoes;
        int[] vetorAcessos;
        int cursor = 0;
        int operacao, posDado;
        ...
        while (cursor < numeroAcessos) {
            operacao = vetorAcessos[cursor];
            posDado = vetorPosicoes[cursor];
